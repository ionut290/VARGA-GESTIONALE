...
function openForm(mode){
  ...
  </div>${mode==='pdf'?'<div id="vgAccountingDocumentSeal"></div>':''}<div class="vg-export-actions"><button type="button" class="ghost" id="vgExportCancel">ANNULLA</button><button type="submit" class="primary">${mode==='excel'?'GENERA EXCEL CLIENTE':'GENERA PDF / STAMPA'}</button></div></form></div>`;
  document.body.appendChild(wrap);document.body.style.overflow='hidden';
  if(mode==='pdf')window.VargaUserDocumentAssets?.mountPicker(wrap.querySelector('#vgAccountingDocumentSeal'),v.documentSeal||{mode:'preset'});
  ...
  wrap.querySelector('form').onsubmit=e=>{e.preventDefault();const data=Object.fromEntries(new FormData(e.currentTarget).entries());data.documentSeal=mode==='pdf'?(window.VargaUserDocumentAssets?.readPicker(wrap.querySelector('#vgAccountingDocumentSeal'))||{mode:'preset'}):{mode:'none'};remember(job,data);close();generate(mode,job,rows,data)};
}
...
function docHtml(job,rows,f){
  ...
  <div class="total"><span>TOTALE LAVORI ESEGUITI</span><span>${money(total)}</span></div>${f.notes?`<div class="notes"><strong>Note:</strong> ${esc(f.notes)}</div>`:''}${sealMarkup}<div class="foot">${esc(f.companyFooter||'')}${f.companyIban?'\nIBAN: '+esc(f.companyIban):''}</div></body></html>`;
}
async function generate(mode,job,rows,data){if(mode==='excel'&&window.VargaInreteAccountingExport?.isInrete(job)){try{await window.VargaInreteAccountingExport.generate(job,rows,data)}catch(err){alert(err?.message||'Esportazione INRETE non riuscita.')}return}let html;try{html=docHtml(job,rows,data)}catch(error){alert(error?.message||'Firma o timbro non disponibili.');return}if(mode==='excel'){const blob=new Blob(['\ufeff'+html],{type:'application/vnd.ms-excel;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`Contabilita-Cliente-${slug(job.title)}.xls`;a.click();URL.revokeObjectURL(a.href);return}const w=window.open('','_blank');if(!w)return alert('Consenti i popup per generare il PDF.');w.document.open();w.document.write(html);w.document.close();setTimeout(()=>{w.focus();w.print()},350)}
