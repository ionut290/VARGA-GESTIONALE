...
const DEPURAZIONE_CONSUNTIVI_ROOT_ID = '1ZTsKCV5zrH2KemJLbgac6pIPRMqye8ku';
...
function driveJobFolder_(payload, createDefaults) {
  ...
  if(createDefaults){const defaultsLock=LockService.getScriptLock();defaultsLock.waitLock(10000);try{['Richieste','Allegati email','Preventivi','Rapportini','Contabilita','MAP','Foto','Documenti'].forEach(n=>driveEnsureUniqueChild_(folder,n))}finally{defaultsLock.releaseLock()}}
  return folder;
}
function driveEnsureUniqueChild_(parent,name){const it=parent.getFoldersByName(name),matches=[];while(it.hasNext())matches.push(it.next());const keep=matches[0]||parent.createFolder(name);matches.slice(1).forEach(folder=>{if(!folder.getFiles().hasNext()&&!folder.getFolders().hasNext())folder.setTrashed(true)});return keep}
...
function saveDepurazioneConsuntivo_(payload){
  ...
  const bytes=Utilities.base64Decode(raw);if(bytes.length>8*1024*1024)throw new Error('Il PDF supera il limite di 8 MB');
  const root=DriveApp.getFolderById(DEPURAZIONE_CONSUNTIVI_ROOT_ID),folderName=dateKey+' - '+plant,folder=getOrCreateFolder_(root,folderName),fileName=dateKey+' - '+plant+'.pdf';
