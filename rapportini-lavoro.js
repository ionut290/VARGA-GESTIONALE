...
(function(){
  ...
  async function store(status,makePdf=false){
    ...
    const job=db.jobs.find(x=>x.id===r.jobId)||{id:'rapportini-generali',title:'Rapportini generali'};
    try{if(status==='Completato')await window.VargaDriveLifecycle?.finalizeJson(r,job,'Rapportino','Rapportini');else await window.VargaDriveLifecycle?.saveDraft(r,job,'Rapportino');save();render()}catch(e){console.warn('Archivio Drive rapportino non disponibile',e);alert('Rapportino salvato nel Gestionale, ma Drive non ha risposto: '+(e.message||e))}
  }
