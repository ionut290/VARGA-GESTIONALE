...
(function installSquadreManagementSync(){
  ...
  const descriptions=value=>{if(value&&typeof value==='object'&&!Array.isArray(value))return value;try{const parsed=JSON.parse(text(value)||'{}');return parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:{}}catch(_){return{}}};
  ...
  function personName(person){const nome=text(person?.nome),cognome=text(person?.cognome),nomeAnagrafica=nome.includes(' ')?nome:'',parti=nome&&cognome&&!normalized(nome).split(' ').includes(normalized(cognome))?`${cognome} ${nome}`:'';return text(parti||nomeAnagrafica||person?.nomeOperatore||person?.operatoreNome||person?.nomeCompleto||person?.displayName||person?.name||nome)}
  function personAliases(person){const nome=text(person?.nome),cognome=text(person?.cognome);return unique([personName(person),person?.nomeCompleto,person?.displayName,person?.nomeOperatore,person?.operatoreNome,person?.name,nome&&cognome?`${nome} ${cognome}`:''].map(text).filter(Boolean))}
  function nameTokens(value){return normalized(value).split(' ').filter(Boolean).sort().join('|')}
  function personRecord(value){const raw=text(value);if(!raw)return null;return(db.vcUtenti||[]).find(person=>personAliases(person).some(alias=>normalized(alias)===normalized(raw)))||(db.vcUtenti||[]).find(person=>personAliases(person).some(alias=>nameTokens(alias)===nameTokens(raw)))||null}
  function canonicalPerson(value){const person=personRecord(value);return person?personName(person):text(value)}
  function personId(person){const source=text(person?.vcSourceId||person?.sourcePath),pathId=source.match(/^personale\/([^/]+)$/)?.[1]||'';return text(person?.uid||person?.userId||person?.idOperatore||person?.operatoreId||pathId||person?.id||person?.sourceId)}
  function personDetail(value){const person=personRecord(value),nome=person?personName(person):text(value);return{id:person?personId(person):'',uid:text(person?.uid||person?.userId),nome,nomeCompleto:nome,email:text(person?.email)}}
  function rowOperatorLinks(row){const capo=personDetail(row.caposquadra),operatori=unique(split(row.personale).map(canonicalPerson)).map(personDetail);return{caposquadra:capo.nome,caposquadraId:capo.id||capo.uid,personale:operatori.map(item=>item.nome).join(', '),personaleIds:unique(operatori.map(item=>item.id||item.uid).filter(Boolean)),personaleDettagli:operatori}}
  ...
  function emptyRow(){return{id:crypto.randomUUID(),caposquadra:'',personale:'',mezzi:'',mezziDescrizioni:{},orario:'',orarioFine:'',senzaPausaPranzo:false,note:''}}
  function readFormRows(){
    return Array.from(document.querySelectorAll('#vgSquadreRows .vg-squad-row')).map(node=>({
      ...
      mezziDescrizioni:descriptions(node.querySelector('[data-field="mezziDescrizioni"]')?.value),
  ...
  function multiPickerMarkup(field,label,current,available,details={}){
    const selected=unique(split(current));
    const choices=unique([...available,...selected]);
    return `<label class="vg-squad-multi-label">${label}<div class="vg-multi-picker" data-multi-picker="${field}"><input data-field="${field}" type="hidden" value="${safe(selected.join(', '))}">${field==='mezzi'?`<input data-field="mezziDescrizioni" type="hidden" value="${safe(JSON.stringify(descriptions(details)))}">`:''}<button class="vg-multi-toggle" type="button" aria-expanded="false"><span>${selected.length?`${selected.length} selezionati`:`Seleziona ${label.toLowerCase()}`}</span><b>⌄</b></button><div class="vg-multi-menu" hidden><input class="vg-multi-search" type="search" placeholder="Cerca ${label.toLowerCase()}..." autocomplete="off"><div class="vg-multi-options">${choices.length?choices.map(value=>`<label><input type="checkbox" value="${safe(value)}"${selected.some(item=>normalized(item)===normalized(value))?' checked':''}><span>${safe(value)}</span></label>`).join(''):`<div class="muted">Nessun elemento disponibile.</div>`}</div><div class="vg-multi-add"><input type="text" placeholder="Aggiungi ${label.toLowerCase().replace('operatori','operatore').replace('mezzi','mezzo')}"><button class="mini" type="button">AGGIUNGI</button></div></div><div class="vg-multi-chips"></div></div></label>`;
  }
  function updateMultiPicker(picker){
    const hidden=picker.querySelector('[data-field]');
    const values=unique(Array.from(picker.querySelectorAll('.vg-multi-options input:checked')).map(input=>input.value));
    const descriptionField=picker.querySelector('[data-field="mezziDescrizioni"]');
    const detailMap=descriptions(descriptionField?.value);picker.querySelectorAll('[data-vehicle-description]').forEach(input=>{const value=text(input.value),key=input.dataset.vehicleDescription;if(value)detailMap[key]=value;else delete detailMap[key]});
    hidden.value=values.join(', ');
    picker.querySelector('.vg-multi-toggle span').textContent=values.length?`${values.length} selezionati`:`Seleziona ${picker.dataset.multiPicker==='personale'?'operatori':'mezzi'}`;
    if(descriptionField)descriptionField.value=JSON.stringify(Object.fromEntries(Object.entries(detailMap).filter(([key])=>values.some(value=>normalized(value)===normalized(key)))));
    picker.querySelector('.vg-multi-chips').innerHTML=values.map(value=>`<span class="${descriptionField?'with-description':''}"><b>${safe(value)}</b>${descriptionField?`<input data-vehicle-description="${safe(value)}" placeholder="Tipo, es. Daily" value="${safe(detailMap[value]||'')}">`:''}<button type="button" data-remove-value="${safe(value)}" aria-label="Rimuovi ${safe(value)}">×</button></span>`).join('');
    picker.querySelectorAll('[data-vehicle-description]').forEach(input=>input.oninput=()=>{const map=descriptions(descriptionField.value),value=text(input.value);if(value)map[input.dataset.vehicleDescription]=value;else delete map[input.dataset.vehicleDescription];descriptionField.value=JSON.stringify(map)});
    picker.querySelectorAll('[data-remove-value]').forEach(button=>button.onclick=()=>{const checkbox=Array.from(picker.querySelectorAll('.vg-multi-options input')).find(input=>normalized(input.value)===normalized(button.dataset.removeValue));if(checkbox)checkbox.checked=false;updateMultiPicker(picker)});
  }
  function bindMultiPickers(node){
    node.querySelectorAll('[data-multi-picker]').forEach(picker=>{
      const toggle=picker.querySelector('.vg-multi-toggle'),menu=picker.querySelector('.vg-multi-menu');
      toggle.onclick=()=>{const open=menu.hidden;menu.hidden=!open;toggle.setAttribute('aria-expanded',String(open))};
      const search=picker.querySelector('.vg-multi-search');search.oninput=()=>{const query=normalized(search.value);picker.querySelectorAll('.vg-multi-options>label').forEach(label=>label.hidden=Boolean(query&&!normalized(label.textContent).includes(query)))};
      picker.querySelectorAll('.vg-multi-options input').forEach(input=>input.onchange=()=>updateMultiPicker(picker));
      const custom=picker.querySelector('.vg-multi-add input'),add=picker.querySelector('.vg-multi-add button');
      add.onclick=()=>{const value=text(custom.value);if(!value)return;let checkbox=Array.from(picker.querySelectorAll('.vg-multi-options input')).find(input=>normalized(input.value)===normalized(value));if(!checkbox){const label=document.createElement('label');label.innerHTML=`<input type="checkbox" value="${safe(value)}" checked><span>${safe(value)}</span>`;picker.querySelector('.vg-multi-options').appendChild(label);checkbox=label.querySelector('input');checkbox.onchange=()=>updateMultiPicker(picker)}else checkbox.checked=true;custom.value='';updateMultiPicker(picker)};
      custom.onkeydown=event=>{if(event.key==='Enter'){event.preventDefault();add.click()}};
      updateMultiPicker(picker);
    });
  }
  function rowMarkup(row,index){
    return `<div class="vg-squad-row" data-row-id="${safe(row.id||crypto.randomUUID())}"><div class="vg-squad-row-head"><strong>Squadra ${index+1}</strong><button class="mini danger" type="button" data-remove-row>RIMUOVI</button></div><div class="vg-squad-row-fields"><label>Caposquadra<input data-field="caposquadra" list="vgSquadrePeople" value="${safe(row.caposquadra)}" placeholder="Nome caposquadra"></label><label class="vg-squad-check"><input data-field="senzaPausaPranzo" type="checkbox"${row.senzaPausaPranzo?' checked':''}> Senza pausa pranzo</label>${multiPickerMarkup('personale','Operatori',row.personale,people())}${multiPickerMarkup('mezzi','Mezzi',row.mezzi,vehicles(),row.mezziDescrizioni)}<label>Ora inizio<input data-field="orario" type="time" value="${safe(row.orario)}"></label><label>Ora fine<input data-field="orarioFine" type="time" value="${safe(row.orarioFine)}"></label><label class="wide">Note<textarea data-field="note" rows="2" placeholder="Note squadra">${safe(row.note)}</textarea></label></div></div>`;
  }
  function renderRows(){
    ...
    bindMultiPickers(node);node.querySelectorAll('[data-remove-row]').forEach(button=>button.onclick=()=>{draftRows=readFormRows().filter(row=>row.id!==button.closest('.vg-squad-row')?.dataset.rowId);renderRows()});
  }
  ...
  function renderSaved(){
    ...
    node.innerHTML=rows.length?rows.map((row,index)=>{const vehicleDetails=descriptions(row.mezziDescrizioni);return `<article class="vg-saved-squad ${row._pending?'is-pending':'is-synced'}"><div class="vg-saved-squad-head"><div><strong>Squadra ${index+1}</strong><span>Caposquadra: ${safe(row.caposquadra||'non indicato')}</span></div><span class="badge ${row._pending?'vg-squad-pending':'vg-squad-origin'}">${row._pending?'DA SINCRONIZZARE':safe(row.createdFrom==='Varga Gestionale'?'GESTIONALE':'VARGA CANTIERI')}</span></div><div class="vg-saved-group"><b>Operatori</b><div>${split(row.personale).length?split(row.personale).map(value=>`<span class="vg-saved-chip">👤 ${safe(value)}</span>`).join(''):'<span class="muted">Nessun operatore</span>'}</div></div><div class="vg-saved-group"><b>Mezzi</b><div>${split(row.mezzi).length?split(row.mezzi).map(value=>`<span class="vg-saved-chip vehicle">🚚 <b>${safe(value)}</b>${vehicleDetails[value]?` · ${safe(vehicleDetails[value])}`:''}</span>`).join(''):'<span class="muted">Nessun mezzo</span>'}</div></div>${row.orario||row.orarioFine?`<div class="vg-saved-meta">🕒 ${safe(row.orario||'--:--')} – ${safe(row.orarioFine||'--:--')}${row.senzaPausaPranzo?' · senza pausa pranzo':''}</div>`:''}${row.note?`<div class="vg-saved-meta">📝 ${safe(row.note)}</div>`:''}</article>`}).join(''):'<div class="empty">Nessuna squadra per questa commessa e questo giorno.</div>';
  }
  function render(){
  ...
  function mergeRows(remoteRows,pendingRows){
    ...
    for(const row of pendingRows||[]){const syncId=text(row.vargaGestionaleId||row.id)||crypto.randomUUID(),links=rowOperatorLinks(row);const next={...links,mezzi:unique(split(row.mezzi)).join(', '),mezziDescrizioni:descriptions(row.mezziDescrizioni),impianti:'',impiantiDettagli:[],note:text(row.note),orario:text(row.orario),orarioFine:text(row.orarioFine),senzaPausaPranzo:Boolean(row.senzaPausaPranzo),conflittiConfermati:{operatori:[],mezzi:[]},conflittoOperatoreConfermato:false,conflittoMezzoConfermato:false,vargaGestionaleId:syncId,createdFrom:'Varga Gestionale'};const position=knownIds.get(syncId);if(position!=null){result[position]={...result[position],...next};updated++;continue}if(knownSignatures.has(signature(next)))continue;result.push(next);knownIds.set(syncId,result.length-1);knownSignatures.add(signature(next));added++}
    return{rows:result,added,updated};
  }
  function repairRowOperators(row){if(text(row?.createdFrom)!=='Varga Gestionale'&&!text(row?.vargaGestionaleId))return{...row};return{...row,...rowOperatorLinks(row)}}
  async function repairCurrentOperators(){
    if(!isAdmin())throw Error('Solo gli amministratori possono riparare le squadre.');if(!cloudStore||!cloudUser)throw Error('Accedi al Cloud con lo stesso account di Varga Cantieri.');const {job,jobId,dateKey}=currentSelection();if(!job||!jobId||!dateKey)throw Error('Seleziona la commessa e il giorno da controllare.');
    const historyRef=cloudStore.collection('squadreStorico').doc(`${dateKey}__${jobId}`),currentRef=cloudStore.collection('squadreCommesse').doc(jobId);let repaired=0,payload=null;
    await cloudStore.runTransaction(async transaction=>{const history=await transaction.get(historyRef),current=await transaction.get(currentRef);if(!history.exists)return;const data=history.data()||{},rows=Array.isArray(data.squadre)?data.squadre:[],next=rows.map(row=>{const fixed=repairRowOperators(row),before=JSON.stringify([row.caposquadra,row.caposquadraId,row.personale,row.personaleIds,row.personaleDettagli]),after=JSON.stringify([fixed.caposquadra,fixed.caposquadraId,fixed.personale,fixed.personaleIds,fixed.personaleDettagli]);if(before!==after)repaired++;return fixed});if(!repaired)return;payload={...data,squadre:next,updatedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedBy:cloudUser.email||'',updatedByUid:cloudUser.uid,updatedFrom:'Varga Gestionale · riparazione ID operatori'};transaction.set(historyRef,payload,{merge:true});const currentData=current.exists?current.data()||{}:{};if(text(currentData.dateKey||currentData.riferimentoData)===dateKey)transaction.set(currentRef,payload,{merge:true})});
    if(payload){const localPayload={...payload,updatedAt:new Date().toISOString()};upsertLocalRecord(`squadreStorico/${dateKey}__${jobId}`,'squadreStorico',`${dateKey}__${jobId}`,localPayload);upsertLocalRecord(`squadreCommesse/${jobId}`,'squadreCommesse',jobId,localPayload);save({skipCloud:true});renderSaved()}return repaired;
  }
  ...
  async function syncPending({silent=false}={}){
    if(busy)return{synced:0,added:0,updated:0};const entries=readPending();if(!entries.length){if(silent)return{synced:0,added:0,updated:0};const repaired=await repairCurrentOperators();setStatus(repaired?`Riparazione completata: ${repaired} ${repaired===1?'squadra aggiornata':'squadre aggiornate'}. Premi Refresh in Varga Cantieri.`:'Le squadre selezionate usano già i nomi corretti.');return{synced:0,added:0,updated:repaired}}if(!isAdmin())throw Error('Solo gli amministratori possono sincronizzare le squadre.');if(!cloudStore||!cloudUser)throw Error('Accedi al Cloud con lo stesso account di Varga Cantieri.');
  ...
  function install(){
    ...
    const section=document.createElement('section');section.id='squadreGestione';section.className='view';section.innerHTML=`<div class="topline vg-squad-head"><div><h1>Organizzazione squadre</h1><p class="subtitle">Prepara le squadre nel gestionale e inviale alla stessa organizzazione usata da Varga Cantieri.</p></div><div class="actions"><button id="vgSquadreSync" class="primary" type="button">SINCRONIZZA CON VARGA CANTIERI</button></div></div><p id="vgSquadrePermission" class="muted"></p><div id="vgSquadreStatus" class="vg-squad-status">Nessuna modifica in attesa.</div><div id="vgSquadreEditor"><div class="panel"><div class="vg-squad-grid"><label>Commessa<select id="vgSquadreJob"></select></label><label>Giorno<input id="vgSquadreDate" type="date"></label></div><div id="vgSquadreRows"></div><div class="actions left"><button id="vgSquadreAddRow" class="ghost" type="button">+ AGGIUNGI SQUADRA</button><button id="vgSquadreSaveDraft" class="primary" type="button">SALVA BOZZA</button></div></div></div><div class="panel"><div class="vg-saved-title"><div><h2>Squadre create</h2><p class="muted">Le squadre blu sono già sincronizzate; quelle gialle devono ancora essere inviate.</p></div><button id="vgSquadreSyncSaved" class="primary" type="button">SINCRONIZZA SQUADRE</button></div><div id="vgSquadreSaved" class="vg-squad-list"></div></div><datalist id="vgSquadrePeople"></datalist><datalist id="vgSquadreVehicles"></datalist>`;main.appendChild(section);
    const runSync=()=>syncPending().catch(error=>setStatus(`Sincronizzazione non riuscita: ${error.message||error}`,'error'));byId('vgSquadreDate').value=tomorrow();byId('vgSquadreJob').onchange=renderSaved;byId('vgSquadreDate').onchange=renderSaved;byId('vgSquadreAddRow').onclick=()=>{draftRows=readFormRows();draftRows.push(emptyRow());renderRows()};byId('vgSquadreSaveDraft').onclick=saveDraft;byId('vgSquadreSync').onclick=runSync;byId('vgSquadreSyncSaved').onclick=runSync;
