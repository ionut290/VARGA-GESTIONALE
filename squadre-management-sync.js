...
(function installSquadreManagementSync(){
  ...
  function updateMultiPicker(picker){
    ...
    picker.querySelector('.vg-multi-chips').innerHTML=values.map(value=>`<span>${safe(value)}<button type="button" data-remove-value="${safe(value)}" aria-label="Rimuovi ${safe(value)}">×</button></span>`).join('');
  ...
  function renderSaved(){
    ...
    node.innerHTML=rows.length?rows.map((row,index)=>`<article class="vg-saved-squad ${row._pending?'is-pending':'is-synced'}"><div class="vg-saved-squad-head"><div><strong>Squadra ${index+1}</strong><span>Caposquadra: ${safe(row.caposquadra||'non indicato')}</span></div><span class="badge ${row._pending?'vg-squad-pending':'vg-squad-origin'}">${row._pending?'DA SINCRONIZZARE':safe(row.createdFrom==='Varga Gestionale'?'GESTIONALE':'VARGA CANTIERI')}</span></div><div class="vg-saved-group"><b>Operatori</b><div>${split(row.personale).length?split(row.personale).map(value=>`<span class="vg-saved-chip">👤 ${safe(value)}</span>`).join(''):'<span class="muted">Nessun operatore</span>'}</div></div><div class="vg-saved-group"><b>Mezzi</b><div>${split(row.mezzi).length?split(row.mezzi).map(value=>`<span class="vg-saved-chip vehicle">🚚 ${safe(value)}</span>`).join(''):'<span class="muted">Nessun mezzo</span>'}</div></div>${row.orario||row.orarioFine?`<div class="vg-saved-meta">🕒 ${safe(row.orario||'--:--')} – ${safe(row.orarioFine||'--:--')}${row.senzaPausaPranzo?' · senza pausa pranzo':''}</div>`:''}${row.note?`<div class="vg-saved-meta">📝 ${safe(row.note)}</div>`:''}</article>`).join(''):'<div class="empty">Nessuna squadra per questa commessa e questo giorno.</div>';
  }
  function render(){
