...
with sync_playwright() as pw:
    ...
    check('Concurrent device update stops stale upload',"""async()=>{seedRemote(testCatalog(4),10);testConnect();await VGIncrementalSync.initialize('varga-azienda:test-user');await VGPriceCatalog.commit(testCatalog(6));seedRemote(testCatalog(8),20);const r=await pushCloudNow(true);return r.ok===false&&db.entries.length===6&&testStore.docs['vargaGestionaleWorkspace_varga-azienda_manifest'].versions.entries===20}""")
    ...
    check('Unsynced local edit survives incoming cloud download',"""async()=>{seedRemote(testCatalog(4),10);testConnect();await VGIncrementalSync.initialize('varga-azienda:test-user');await VGPriceCatalog.commit(testCatalog(6));seedRemote(testCatalog(8),20);const r=await VGIncrementalSync.applyManifest(testSnapshot(),'varga-azienda:test-user');return r.conflict===true&&db.entries.length===6&&VGPriceCatalog.status().pending}""")
