...
extra = r'''
    ...
    check('Retry rejects overlapping price changes without overwriting either version',"""async()=>{seedRemote(testCatalog(4),10);testConnect();await VGIncrementalSync.initialize('varga-azienda:test-user');const c=testCatalog(4);c.entries[0].price=999;await VGPriceCatalog.commit(c);const remote=testCatalog(4);remote.entries[0].price=888;seedRemote(remote,20);const result=await retryPriceCatalog();return !result.ok&&result.code==='PREZZIARI_CONFLITTO'&&db.entries[0].price===999&&testStore.docs['vargaGestionaleWorkspace_varga-azienda_manifest'].versions.entries===20}""")
