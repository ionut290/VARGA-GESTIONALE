(() => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      .verde-bologna-overview-marker-wrap{background:transparent!important;border:0!important}.verde-bologna-overview-marker{display:inline-flex;align-items:center;justify-content:center;gap:3px;min-width:30px;height:30px;padding:0 6px;border:2px solid #fff;border-radius:15px;color:#fff;box-shadow:0 2px 6px rgba(0,0,0,.42);font-size:1rem;font-weight:900;white-space:nowrap}.verde-bologna-overview-marker.has-code{height:26px;font-size:.72rem}.verde-bologna-overview-marker.is-hedge{min-width:42px;border-radius:8px;background:#3d8c40!important;letter-spacing:-2px}
  ...
  function addOverviewGeometry(dataset, record, index, group, showCode) {
    ...
    try {
      ...
      const markerIcon = () => L.divIcon({ className: "verde-bologna-overview-marker-wrap", html: `<span class="verde-bologna-overview-marker${showCode && code.official ? " has-code" : ""}${dataset.id === "siepi" ? " is-hedge" : ""}" style="background:${color}">${dataset.id === "siepi" ? "🌿🌿" : dataset.icon}${showCode && code.official ? ` ${esc(code.value)}` : ""}</span>`, iconSize: null, iconAnchor: [18, 14], popupAnchor: [0, -15] });
      ...
      if (geometry.type !== "Point" && geometry.type !== "MultiPoint") attachPopup(L.marker([center.lat, center.lon], { icon: markerIcon(), keyboard: true, riseOnHover: true, title: `${code.official ? code.value + " · " : ""}${title}` }).addTo(group));
      return true;
