...
function ensureStyles(){
  ...
  style.textContent=`
.vg-view-scene{--scene:#176b48;position:relative;flex:1;min-width:220px;max-width:640px;height:76px;margin:0 22px;overflow:hidden;border:1px solid color-mix(in srgb,var(--scene) 22%,#dce8e1);border-radius:16px;background:linear-gradient(135deg,color-mix(in srgb,var(--scene) 9%,white),#fff);box-shadow:inset 0 -7px 0 color-mix(in srgb,var(--scene) 9%,transparent)}
...
.vg-scene-items{position:absolute;left:27%;right:7%;top:13px;bottom:20px;display:flex;align-items:center;justify-content:space-around}.vg-scene-item{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;background:#fff;border:1px solid color-mix(in srgb,var(--scene) 22%,#dbe6df);color:var(--scene);font-weight:900;font-size:18px;box-shadow:0 4px 10px rgba(20,50,35,.08);animation:vgSceneFloat 3s ease-in-out infinite}.vg-scene-item:nth-child(2){animation-delay:.35s}.vg-scene-item:nth-child(3){animation-delay:.7s}
